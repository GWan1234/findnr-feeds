...
local m = Map("simple2fa", translate("双因素认证"), translate("启用二次验证以保护您的路由器登录。"))
...
local s = m:section(NamedSection, "global", "settings", translate("设置"))

-- === 2. 功能开关 ===
s:option(Flag, "enabled", translate("启用二次验证"))

-- === 3. 显示密钥 (带复制功能) ===
local o = s:option(DummyValue, "_secret_display", translate("密钥"))
o.description = translate("如果无法扫描二维码，请手动输入此密钥。")
o.rawhtml = true
o.cfgvalue = function(self, section)
    ...
    return string.format([[
        <div style="display: flex; align-items: center;">
            ...
            <input type="button" class="cbi-button cbi-button-apply" value="%s" onclick="
    ...
    ]], val, translate("复制"), translate("已复制到剪贴板"))
end

-- === 4. 刷新密钥按钮 ===
local btn = s:option(Button, "_refresh", translate("刷新密钥"))
btn.inputstyle = "remove"
btn.description = translate("警告：刷新密钥后，您必须重新配置所有设备的验证器应用。")
...
local qr = s:option(DummyValue, "_qrcode", translate("扫描二维码"))
qr.description = translate("请使用 Google Authenticator、Authy 或 Microsoft Auth 扫描此二维码。")
